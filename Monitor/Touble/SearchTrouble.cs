...
{
    public partial class SearchTrouble : MonitorForm
    {
        public SearchTrouble(Index owner)
            : base(owner)
        {
            InitializeComponent();
            winFormPager1.PageSize = 20;
            this.owner = owner;
            owner.ShowInfo("已确认故障");
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                dt = sqlHelper.ExecuteQueryDataTable("select * from v_alarm_log where start_time between '" + dateTimePicker1.Value + "' and '" + dateTimePicker2.Value + "' and status_type_name='已确认'" + s);//MergeQuery.GetDataRange("v_alarm_log", "*", "start_time", dateTimePicker1.Value, dateTimePicker2.Value, "status_type_name='已确认'" + s);
            }
            int pageCount;
            string msg;
            ...
            if (dv.Count > 0)
            {
                ...
                object target = owner;
                CallbackArgs callback = new CallbackArgs(dlgt, target, new object[] { "已确认故障", "$SdgReport|{FLASHTIME},{TRAINNO}$" });
                string columnKey;
        ...
        {
            Exporter.ExportToExcel(outlookGrid1.DataSource as DataTable, "已确认故障报表");
        }
