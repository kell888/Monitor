...
{
    /// <summary>
    /// 问题：图片是以“前后倾斜量”的监测类型来找出来的！！！
    /// </summary>
    public partial class FB_Angle : Form
    {
        public FB_Angle(Index owner, DateTime time, string device_name, string station_name, bool direction, string train_no, int point_type_id)
        {
            InitializeComponent();
            this.time = time;
        ...
        private void FB_Angle_Load(object sender, EventArgs e)
        {
            ...
            if (current < morning || current >= nightfall)//判断是白天还是晚上（晚上）
            {
                try
                {
                    DataTable dt = MergeQuery.GetDataBefore("v_data_log", "top 2 data_value,filepath,flash_time,status_type_name", "flash_time", time, "and device_name ='" + device_name + "' and train_no ='" + train_no + "'  and direction ='" + direction + "' and (convert(varchar(8),flash_time,114) between '" + night1 + "' and '24:00:00' or convert(varchar(8),flash_time,114) between '00:00:00' and '" + night2 + "') and point_type_name='中心偏移平均值' and point_type_id=" + point_type_id + " and (status_type_name<>'警示' and status_type_name<>'疑似报警'", "order by flash_time DESC", null);
                    if (dt.Rows.Count > 0)
                    {
                    ...
                    dt = MergeQuery.GetDataBefore("v_data_log", "top 2 data_value,filepath,flash_time,status_type_name", "flash_time", time, "and device_name ='" + device_name + "' and train_no ='" + train_no + "'  and direction ='" + direction + "' and (convert(varchar(8),flash_time,114) between '" + night1 + "'and'24:00:00' or convert(varchar(8),flash_time,114) between '00:00:00'and'" + night2 + "') and point_type_name='前后倾斜量' and point_type_id=" + point_type_id + " and (status_type_name<>'警示' and status_type_name<>'疑似报警'", "order by flash_time DESC", null);
                    if (dt.Rows.Count > 0)
                    {
