...
namespace Monitor.subroutine_page
{
    ...
    {
        public Processing(Index owner, string time, string device_name)
        ...
        {
            DataTable dt = MergeQuery.GetDataAt("v_alarm_log", "status_type_name,start_time,address,train_no,device_name,alarm_value,point_type_name", "Start_time", Convert.ToDateTime(time), "device_name='" + device_name + "'", null, null);
        ...
        {
            DataTable dt = MergeQuery.GetDataAt("v_alarm_log", "id", "Start_time", Convert.ToDateTime(time), "device_name='" + device_name + "'", null, null);
            int id = 0;
            if (dt.Rows.Count > 0)
            {
                DateTime t = Convert.ToDateTime(time);
                id = Convert.ToInt32(dt.Rows[0][0]);
                MergeUpdate.UpdateData("d_alarmAction_log", "affirmance=2, process_time='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',alarm_msg='" + textBox1.Text + "',processer_id=" + Index.User.ID + ",process_msg='" + textBox2.Text + "'", "id", id.ToString(), MergeCommon.GetYYYYMM(t));
