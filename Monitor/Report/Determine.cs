...
{
    public partial class Determine : MonitorForm
    {
        public Determine(Index owner, DateTime time, string device_name)
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            using (SqlHelper sqlHelper = new SqlHelper())
            {
                ...
                if (o != null && o != DBNull.Value)
                {
                    ...
                    if (sqlHelper.ExecuteNonQuery("update d_alarmAction_log set confirmer_id=" + Index.User.ID + ", affirmance=1, confirm_time='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' where id=" + id) > 0)
                    {
