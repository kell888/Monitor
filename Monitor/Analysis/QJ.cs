...
{
    //球铰高度的图片名为1
    public partial class QJ : Form
    {
        public QJ(Index owner, int id)
        {
            InitializeComponent();
            this.id = id;
        }
        ...
        private void FB_Angle_Load(object sender, EventArgs e)
        {
            ...
            using (SqlHelper sqlHelper = new SqlHelper())
            {
                ...
                if (current < morning || current >= nightfall)//判断是白天还是晚上（晚上）
                {
                    try
                    {
                        dt = sqlHelper.ExecuteQueryDataTable("select top 4 device_name,data_value from v_data_log where flash_time <= '" + time + "' and train_no = '" + train_no + "' and (convert(varchar(8),flash_time,114) between '" + night1 + "' and '24:00:00' or convert(varchar(8),flash_time,114) between '00:00:00'and'" + night2 + "') and point_type_name='" + point_type_name + "'" + " and not(status_type_name='警示' or status_type_name='疑似报警') order by flash_time DESC");//MergeQuery.GetDataBefore("v_data_log", "top 4 device_name,data_value", "flash_time", time, "train_no = '" + train_no + "' and (convert(varchar(8),flash_time,114) between '" + night1 + "' and '24:00:00' or convert(varchar(8),flash_time,114) between '00:00:00'and'" + night2 + "') and point_type_name='" + point_type_name + "'" + " and not(status_type_name='警示' or status_type_name='疑似报警')", "flash_time DESC", null);
                        if (dt.Rows.Count > 0)
                        {
                        ...
                        dt = sqlHelper.ExecuteQueryDataTable("select filepath,flash_time from v_data_log where flash_time <= '" + time + "' and train_no = '" + train_no + "' and (convert(varchar(8),flash_time,114) between '" + night1 + "'and'24:00:00' or convert(varchar(8),flash_time,114) between '00:00:00'and'" + night2 + "') and point_type_name='" + point_type_name + "'" + " and not(status_type_name='警示' or status_type_name='疑似报警') order by flash_time DESC");//MergeQuery.GetDataBefore("v_data_log", "filepath,flash_time", "flash_time", time, "train_no = '" + train_no + "' and (convert(varchar(8),flash_time,114) between '" + night1 + "'and'24:00:00' or convert(varchar(8),flash_time,114) between '00:00:00'and'" + night2 + "') and point_type_name='" + point_type_name + "'" + " and not(status_type_name='警示' or status_type_name='疑似报警')", "flash_time DESC", null);
                        if (dt.Rows.Count > 0)
                        {
