...
{
    //磨耗缺口的图片名为A_2,B_2,A_1,B_1，且缺口的数据文本gaplist_1.txt和gaplist_4.txt也放在图片目录下
    public partial class MHQK : Form
    {
        public MHQK(Index owner, int id)
        {
            InitializeComponent();
            this.id = id;
        }
        ...
        private void FB_Angle_Load(object sender, EventArgs e)
        {
            ...
            using (SqlHelper sqlHelper = new SqlHelper())
            {
                ...
                if (current < morning || current >= nightfall)//判断是白天还是晚上（晚上）
                {
                    try
                    {
                        DataTable dt = sqlHelper.ExecuteQueryDataTable("select top 2 data_value,filepath,flash_time,status_type_name from v_data_log where flash_time <= '" + time + "' and device_name ='" + device_name + "' and train_no ='" + train_no + "'  and direction ='" + direction + "' and (convert(varchar(8),flash_time,114) between '" + day + "'and'" + night1 + "') and point_type_name='" + point_type_name + "'" + " and (status_type_name<>'警示' and status_type_name<>'疑似报警') order by flash_time DESC");//MergeQuery.GetDataBefore("v_data_log", "top 2 data_value,filepath,flash_time,status_type_name", "flash_time", time, "device_name ='" + device_name + "' and train_no ='" + train_no + "'  and direction ='" + direction + "' and (convert(varchar(8),flash_time,114) between '" + day + "'and'" + night1 + "') and point_type_name='" + point_type_name + "'" + " and (status_type_name<>'警示' and status_type_name<>'疑似报警')", "flash_time DESC", null);
                        if (dt.Rows.Count > 0)
                        {
                            if (point_type_name.Contains("磨耗"))
                            {
                        ...
                        if (ddt.Rows.Count > 0)
                        {
                            ...
                            if (point_type_name.Contains("缺口"))
                            {
                                string fileName = Environment.CurrentDirectory + "/Image/" + day1 + "/" + fb + "/gaplist_1.txt";
                                if (pt == "B_")
                                    fileName = Environment.CurrentDirectory + "/Image/" + day1 + "/" + fb + "/gaplist_4.txt";
